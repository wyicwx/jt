...
function getProvision(u, list) {
	...
	function map(o) {
		...
		if(jt.utils.isString(o)) {
			...
			o.value = o.value.replace(/\$\d/g, replace);
		} else if(o.file) {
			o.file = o.file.replace(/\$\d/g, replace);
		}
		return o;
	}
	...
	for (var i in list) {
		if (list[i].match.test(parse.href)) {
			...
			provision.respond = provision.respond.map(map);
			break;
		}
	}
	return provision;
...
function startServer() {
	...
	http.createServer(function(request, response) {
		...
		if(request.url.search(/127.0.0.1|localhost/) == -1) {
			// 转发请求
...
jt.commander.command({
	...
	handler: function(argv) {
		if(argv['-'] == 'start') {
			...
			var mathcs = jt.server.getMatchRule();
