...
(function() {
	...
	jt.utils.each(list, function(project, key) {
		...
		jt.utils.each(project.files, function(toCombo, fileName) {
			...
			if(project.output) {
				outPath = jt.utils.realPath(project.output);
				fullpath = path.join(outPath, fileName);
				outputAlias[fullpath] = path.join(outPath, fileName);
			} else{
				outPath = path.join(jt.root, config.outputPath);
...
function _fileGeter(filePath, callback) {
	if(fs.existsSync(filePath)) {					//绝对路径文件
		...
		var rePath = path.join(config.basePath, filePath);
 ...
 * @param  {Object}   config   
 * @config projects	{boolean}  true   搜索项目内文件,default true 
 * @config pathFile	{boolean}  true   搜索目录下文件,default true
...
builder.finder = function(files, cfg, callback) {
	...
	jt.utils.each(files, function(file) {
		...
		if(cfg.pathFile) {
			app.do(function(done) {
				_searchFile(config.basePath, file, function(files) {
 ...
 * @config {Boolean} notDefaultOutput 不使用默认输出地址
 */
builder.save = function(info, cfg) {
	...
	jt.utils.each(info, function(fileBuffer, filePath) {
		...
		// builder内定义的文件地址映射
			...
			if(!cfg.notDefaultOutput) {
				// 默认规则：替换项目地址到output地址
				if(!filePath.indexOf(config.basePath)) {
					filePath = filePath.replace(config.basePath, '');
				}
				filePath = path.join(jt.root, config.outputPath, filePath);
			}
		}
