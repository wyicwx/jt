...
ReadStream.processStream = function(options, quote, isResolveFile) {
	...
	if(!isResolveFile && options.file) {
		options.file = ReadStream.resolveProcessor(options.file);
	}
	// 实现buffer
	if(options.file) {
	...
	var result = []
	options.file.forEach(function(origin) {
		var o = aggre(origin);
		processors.forEach(function(p) {
			var handler = p[1],
				name = p[0];

			o = o.pipe(handler(jt.utils.result(options, name), {dir: options.dir}));
		});
		result.push(o);
	});
