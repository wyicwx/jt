...
function _map2combo(paths, ignore) {
	...
	jt.utils.each(paths, function(combo) {
		if(jt.utils.isString(combo)) {
			...
			if(combo in config.fs && !ignore[combo]) {
				ignore[combo] = true;
...
function _readSimpleFile(file, callback) {
	if(jt.utils.isString(file)) {
		fs.readFile(p, function(err, buffer) {
			if(err) {
				console.log('['+'warnning'.yellow+'] not found "'+p+'", return "".');
				callback('');
			} else {
				callback(buffer);
			}
...
vfs.processorDefine = function(name, handler) {
	this._pushPipe('__vfs_processor', function(data) {
...
vfs.isVirtual = function(p) {
	return _pathConverter(p) in config.fs;
};
