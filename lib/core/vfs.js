...
VFS.prototype = {
	...
	isResolved: function(filePath) {
		filePath = this.pathResolve(filePath);

		return filePath in this.resolvedPath;
	},
...
newReadStream.processorType._init = function() {
	this._setupInfo();
	if(this.type == newReadStream.fileType) {
		this._readFile();
	} else {
...
newReadStream.prototype._setupInfo = function() {
	if(_.iString(filecontent)) {
		this.type = newReadStream.fileType;
		...
		this.type = newReadStream.processorType;
	}
...
ReadStream.prototype._resolve = function() { // 解析配置
	if(!this.resolved) {
...
newReadStream.processorType = 1;
newReadStream.fileType = 0;
