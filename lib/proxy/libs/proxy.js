...
/**
 * 根据参数获取http头,返回无缓存头部文件
 */
exports.getHttpHeader = function(opt) {
	var ext, headers;
...
exports.reply = function(request, response) {
	...
	if(hosts[urlParse.host]) { // host支持
		options.host = hosts[urlParse.host];
		delete options.hostname;
	} else { // 转发请求
		if(jt.config.proxy.agent && jt.config.proxy.agent.host) { // 对另一个代理服务器的支持
			// path 指定完整url
	...
	req.on('response', function(res) {
		...
		if(config.slowSpeedSimulate) {
			var tStream = new jt.utils.TrickleStream({
				size: config.slowBlockByte,
				interval: config.slowTimeInterval
			});
